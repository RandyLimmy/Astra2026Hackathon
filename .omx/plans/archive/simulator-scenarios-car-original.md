# Original car simulator scenario plan (archived)

Historical design retained for the S0–S6 car experiments. The current scope and delivery order are in `../simulator-scenarios.md`; its four failure demos and tooling contract supersede this document's car-only scope and exclusion of tools. Statements below describe the original planning-time state, not the current checkout.
