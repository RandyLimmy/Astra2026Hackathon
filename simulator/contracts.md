...
The viewer opens paused. **Space** starts the trial, pauses/resumes while it runs, and starts a full replay after completion. **R** repeats the current trial while retaining heat and completed faults, including detached wheels. **N** restarts the original experiment from its initial fault state and ambient temperature, replaying its declared conditioning/recovery. All resets keep the same viewer window. Escape closes it. A finished trial holds its final pose until replay or close. Viewer runs do not automatically save telemetry; use `run --frames` for recordings.
